...
async def start(event):
  await event.reply("**Majeste Tagger🇹🇷**, Grup veya kanaldaki neredeyse tüm üyelerden bahsedebilirim ★\nDaha fazla bilgi için **/bilgi**'i tıklayın.",
                    buttons=(
                      [
                         Button.url('➕ BENİ GRUBA EKLE ➕ ', 'http://t.me/Majeste_taggerbot?startgroup=a')
                      ],
                      [
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("İşlem Başarılı Bir Şekilde Durduruldu ❌ **Resmi Kanal @MajesteBotlar**")
        return
      if usrnum == 5:
